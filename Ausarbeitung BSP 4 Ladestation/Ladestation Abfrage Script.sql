...
SELECT lv.idLadevorgang, lv.Startzeit, lv.Endzeit, TIMEDIFF(lv.Endzeit, lv.Startzeit) AS ladezeit, lv.Kilowattstunden AS 'geladene Kilowattstunden', t.idTarif, ls.Status, (Kilowattstunden * PreisProStunde) + t.Startgebühr AS preis, k.`E-Mail-Adresse` FROM ladevorgang lv JOIN tarif t ON lv.fkTarif = t.idTarif JOIN ladestation ls ON lv.fkLadestation = ls.Stationsnummer JOIN kunde k ON lv.fkKunde = k.Kundennummer;

SELECT Kundennummer, Vorname, Nachname, `E-Mail-Adresse` FROM kunde;
SELECT idTarif, PreisProStunde, Startgebühr FROM tarif;
SELECT ls.Stationsnummer, ls.Bezeichnung, ls.Status FROM ladestation ls JOIN ladevorgang lv ON lv.
SELECT
        ls.Stationsnummer,
        ls.Bezeichnung
     FROM Ladestation ls
     WHERE NOT EXISTS (
         SELECT *
         FROM Ladevorgang lv
         WHERE lv.fkLadestation = ls.Stationsnummer
         AND lv.Endzeit IS NULL
     )
     ORDER BY ls.Bezeichnung
