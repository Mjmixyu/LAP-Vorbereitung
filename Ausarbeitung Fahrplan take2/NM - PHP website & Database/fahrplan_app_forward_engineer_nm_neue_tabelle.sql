...
-- -----------------------------------------------------
-- Table `fahrplan_app`.`Sitzplatzreservierung`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `fahrplan_app`.`Sitzplatzreservierung` (
  `idSitzplatzreservierung` INT NOT NULL AUTO_INCREMENT,
  `platznummer` VARCHAR(10) NOT NULL,
  `FK_idFahrt` INT NOT NULL,
  PRIMARY KEY (`idSitzplatzreservierung`),
  INDEX `FK_idFahrtReservierung_idx` (`FK_idFahrt` ASC),
  UNIQUE INDEX `UQ_fahrt_platz` (`FK_idFahrt` ASC, `platznummer` ASC),
  CONSTRAINT `FK_idFahrtReservierung`
    FOREIGN KEY (`FK_idFahrt`)
    REFERENCES `fahrplan_app`.`Fahrt` (`idFahrt`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
